...
fn test_wrong_password_returns_error() {
    ...
    table.insert(RedbxValue::U64(1), RedbxValue::U64(1)).unwrap();
    txn.commit().unwrap();
    drop(db);
...
fn test_get_missing_key_returns_none() {
    ...
    let txn = db.begin_write().unwrap();
    let table = txn.clone().open_table("kv".to_string()).unwrap();
    table.insert(RedbxValue::Str("seed".to_string()), RedbxValue::U64(0)).unwrap();
    txn.commit().unwrap();

    let rtxn = db.begin_read().unwrap();
    let rtable = rtxn.clone().open_table("kv".to_string()).unwrap();
    let val = rtable
        .get(RedbxValue::Str("missing".to_string()))
        .unwrap();
    assert_eq!(val, None);
}
...
fn test_all_value_types_roundtrip() {
    ...
    for (i, v) in values.iter().enumerate() {
        table
            .insert(RedbxValue::U64(i as u64), v.clone())
            .unwrap();
    }
    txn.commit().unwrap();
...
fn test_multimap_insert_and_get() {
    ...
    let rtable = rtxn
        ...
        .open_multimap_table("tags".to_string())
        .unwrap();
    let mut values = rtable
        .get(RedbxValue::Str("post1".to_string()))
        .unwrap();
...
fn test_multimap_remove_single() {
    ...
    let table = txn
        .clone()
        .open_multimap_table("mm".to_string())
        .unwrap();
    table
