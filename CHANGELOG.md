...
### 🔄 Upstream Sync: redb v3.1.0
Synced with upstream redb v3.1.0 to incorporate latest improvements while maintaining encryption features.
...
* Performance improvements from redb v3.0.2:
  - Fix performance issue where transactions with large writes caused slow `WriteTransaction::abort()`
  - Improved cache flush performance

#### redbx-Specific Updates:
* **Maintained full encryption compatibility** - All encryption features work seamlessly with upstream changes
* **Updated FileBackend integration** - Properly integrated new `FileBackend::new_internal()` method
